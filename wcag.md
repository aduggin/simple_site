...
##### Common issues
* Table headings are not used correctly
* A list of items is presented using line breaks to separate each item - the proper HTML markup has not been used;
* Headings are created without using the correct markup, for example ```<strong>``` instead of ```<h3>```
